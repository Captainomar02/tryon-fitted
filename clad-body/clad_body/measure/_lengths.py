...
from ._slicer import MeshSlicer, WAIST_HEIGHT_PCT


# Standard untucked shirts can fall back to mid-fly when hip height is
# unavailable, but the main measurement path should terminate at the measured
# hip line. Longline or outerwear fits should use a garment-specific hem target
# instead of reusing this standard-top fallback ratio.
STANDARD_SHIRT_HEM_FALLBACK_WAIST_TO_CROTCH_RATIO = 0.5
...
def measure_shirt_length(joints, mesh, crotch_z, measurements=None, step=0.005):
    """Measure shirt length — shoulder to hip line along front contour.

    Standard untucked shirts should cover the waistband and end at the hip
    line for this pipeline. The measured hip height already exists as an
    anatomical reference and matches the red contour used in the render.
    ...
    Algorithm:
      ...
      2. **Hem target** — use the measured hip height when available so the
         shirt-length line ends on the same red hip contour shown in the
         visualisation. If hip metadata is unavailable, fall back to a
         waist-to-crotch midpoint estimate.
      3. **Front surface sweep** — from 5 cm below start Z to the hem target
         (5 mm steps). At each Z, take most-anterior Y within ±4 cm of
         ``trace_x``.
      4. **Convex hull ("not tight to skin")** — lower convex hull of the
         (Z, Y) profile via Andrew's monotone chain.  Bridges concavities,
         follows convex curvature back toward the body after peaks.
      5. **Single path** — same polyline for measurement and visualisation.
    ...
    hip_z = float(measurements.get("_hip_z", 0) or 0)
    if hip_z > crotch_z:
        hem_target_z = hip_z
    else:
        waist_z = float(measurements.get("_waist_z", 0) or 0)
        if waist_z <= crotch_z:
            height = float(verts[:, 2].max())
            waist_z = height * WAIST_HEIGHT_PCT
        if waist_z <= crotch_z:
            return 0, None
        hem_target_z = waist_z - (
            (waist_z - crotch_z) * STANDARD_SHIRT_HEM_FALLBACK_WAIST_TO_CROTCH_RATIO
        )
    if snz <= hem_target_z:
        return 0, None

    # ── Front surface sweep (5 cm below shoulder → hip-line hem target) ──
    # Start point is prepended; hull bridges from shoulder to first bust contact.
    x_band = 0.04
    sweep_top = snz - 0.05
    sweep_bottom = hem_target_z
